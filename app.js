...
const A = {
  ...
  payBill(id){
    ...
    const body='<div style="text-align:center;padding:4px 0 14px">'
      ...
      +(isMobile?'':'<img src="'+qrUrl+'" alt="UPI QR" style="width:200px;height:200px;border-radius:12px;border:3px solid var(--acc);background:#fff;padding:4px"><br>')
      +'<div style="margin-top:10px;font-size:.875rem;color:var(--txt2)">'+(isMobile?'Tap <strong>Open UPI App</strong> below to pay directly':'Scan QR with <strong>GPay / PhonePe / Paytm</strong>')+'</div>'
      +'<div style="margin-top:8px;background:var(--inp);border-radius:8px;padding:8px 14px;display:inline-flex;align-items:center;gap:10px"><div><div style="font-size:.68rem;color:var(--mute);margin-bottom:2px">UPI ID</div><div style="font-family:monospace;font-weight:700;color:var(--acc)">'+upi+'</div></div><button class="btn btn-sm btn-s" onclick="navigator.clipboard.writeText(\''+upi+'\').then(()=>A.toast(\'Copied UPI ID!\',\'ok\'))"><span class="material-icons-round" style="font-size:16px">content_copy</span></button></div></div>'
      +'<div style="margin-bottom:10px;padding:10px;background:rgba(255,181,71,.08);border:1px solid rgba(255,181,71,.3);border-radius:8px;font-size:.8rem;color:var(--warn)">After paying, enter the UTR number below for admin verification.</div>'
    ...
    const foot='<button class="btn btn-s" onclick="A.closeModal()">Cancel</button>'
      +(isMobile?'<a href="'+upiUrl+'" class="btn btn-p" style="text-decoration:none"><span class="material-icons-round">open_in_new</span>Open UPI App</a>':'<button class="btn btn-s" onclick="navigator.clipboard.writeText(\''+upi+'\').then(()=>A.toast(\'UPI ID Copied! Paste in GPay/PhonePe\',\'ok\')))"><span class="material-icons-round">content_copy</span>Copy UPI ID</button>')
  ...
  rAnalytics(){
    ...
    <div class="card" style="margin-top:14px"><div class="ch"><h3>Tenant Management</h3><span class="badge b-ok">${d.pharmacies.filter(p=>p.status==='active').length} Active</span></div><div class="tw"><table><thead><tr><th>Pharmacy</th><th>Email</th><th>Plan</th><th>Status</th><th>Revenue</th><th>Joined</th><th>Actions</th></tr></thead><tbody>${d.pharmacies.map(p=>{const rev=d.bills.filter(b=>b.phId===p.id&&b.status==='paid').reduce((s,b)=>s+b.amt,0);const pend=d.bills.filter(b=>b.phId===p.id&&b.status==='unpaid').reduce((s,b)=>s+b.amt,0);return`<tr><td><div style="font-weight:700">${p.name}</div></td><td style="font-size:.8rem;color:var(--mute)">${p.email}</td><td>${p.plan?`<span class="badge b-acc">₹${p.plan}/mo</span>`:'<span class="badge b-gray">No Plan</span>'}</td><td>${this.sbadge(p.status)}</td><td><div style="font-weight:700;color:var(--ok)">₹${this.fmt(rev)}</div>${pend>0?`<div style="font-size:.72rem;color:var(--err)">₹${this.fmt(pend)} pending</div>`:''}</td><td style="font-size:.8rem;color:var(--mute)">${p.joined}</td><td><div class="ta"><button class="btn btn-sm btn-s" onclick="A.phDetail('${p.id}')">View</button><button class="btn btn-sm btn-p" onclick="A.editPhModal('${p.id}')">Edit</button></div></td></tr>`;}).join('')}</tbody></table></div></div>`;
  },

  async loadAnalytics(){
    const res=await apiGet('/admin/analytics');if(!res)return;
    const set=(id,v)=>{const el=Q('#'+id);if(el)el.textContent=v;};
    set('an-ph',res.totalPharmacies);
    set('an-ph2',`${res.activePharmacies} active · ${res.pendingPharmacies} pending`);
    set('an-rev','₹'+this.fmt(res.totalRevenue));
    set('an-rev2','+₹'+this.fmt(res.pendingRevenue)+' pending');
    set('an-sub',res.premiumSubs+res.basicSubs);
    set('an-sub2',`${res.premiumSubs} premium · ${res.basicSubs} basic`);
